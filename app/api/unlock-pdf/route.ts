...
export async function POST(req: NextRequest) {
  try {
    ...
    return pdfResponse(Buffer.from(bytes), "unlocked.pdf");
  } catch (e: unknown) {
    return NextResponse.json({ error: e instanceof Error ? e.message : "Failed to unlock PDF. Check your password." }, { status: 500 });
  }
}
